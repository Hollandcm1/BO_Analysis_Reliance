...
# ANOVA
model <- aov(data = data_long, Confidence ~ Condition * Block + Error(Participant / Block))
...
# Visualize Data
g1 <- ggplot(data_long, aes(x=Block, y=Confidence, color=Condition, group=Condition)) + 
  geom_smooth(alpha = 0.25) + 
  geom_jitter(width = 0.2, height = 0) +
  theme_classic() +
  labs(title = "Confidence by Block by Condition", x = "Block", y = "Confidence") +
  scale_x_continuous(breaks = seq(1, 6, 1)) +
  scale_color_manual(values = c("Decreasing" = colour.decreasing, "Increasing" = colour.increasing))+
  ylim(0, 100)
print(g1)
ggsave(here('output','figures','07_Confidence_by_Block_by_Condition.png'), 
...
g2 <- ggplot(summary_data, aes(x = Block, y = Average_Confidence, group = Condition, color = Condition)) +
  geom_line() +
  geom_point(position = position_dodge(width = dodge_width)) +
  geom_errorbar(aes(ymin = Average_Confidence - se, ymax = Average_Confidence + se), 
                width = 0.4, position = position_dodge(width = dodge_width)) +
  theme_classic() +
  scale_color_manual(values = c("Decreasing" = colour.decreasing, "Increasing" = colour.increasing)) +
  labs(title = "Average Confidence across Conditions with Standard Error",
       x = "Block",
       y = "Confidence",
       color = "Condition") +
  ylim(0, 100) + 
...
print(g2)
ggsave(here('output','figures','07_Confidence_by_Block_by_Condition_with_SE.png'), 
       plot = g2, device = device, width = width, height = height, units = units, dpi = dpi)

# Visualize Interaction with Standard Error as Ribbon
dodge_width <- 0.2 # seperation between conditions
g3 <- ggplot(summary_data, aes(x = Block, y = Average_Confidence, group = Condition, color = Condition, fill = Condition)) +
  geom_line() +
  geom_point(position = position_dodge(width = dodge_width)) +
  ...
  labs(title = "Average Confidence across Conditions with Standard Error as Ribbon",
       x = "Block",
       y = "Confidence",
       color = "Condition") +
  ylim(0, 100) + 
  ...
  geom_point(data = data_long, aes(x = Block, y = Confidence, color = Condition), 
             position = position_jitterdodge(jitter.width = 0.2, dodge.width = dodge_width), alpha = 0.5)
print(g3)
ggsave(here('output','figures','07_Confidence_by_Block_by_Condition_with_SE_as_Ribbon.png'), 
