...
# ANOVA
model <- aov(data = data_long, Performance_Before ~ Condition * Block + Error(Participant / Block))
model_summary <- summary(model)
capture.output(model_summary, file = here('output','tables','05_Performance_Before_Simple_Anova.txt'))
...
# Visualize Data
g1 <- ggplot(data_long, aes(x=Block, y=Performance_Before, color=Condition, group=Condition)) + 
  geom_smooth(alpha = 0.25) + 
  geom_jitter(width = 0.2, height = 0) +
  theme_classic() +
  labs(title = "Performance Before by Block by Condition", x = "Block", y = "Performance Before") +
  scale_x_continuous(breaks = seq(1, 6, 1)) +
  scale_color_manual(values = c("Decreasing" = colour.decreasing, "Increasing" = colour.increasing))+
  ylim(0, 1) +
  theme(plot.title = element_text(size=title_size))
print(g1)
ggsave(here('output','figures','05_Performance_Before_by_Block_by_Condition.png'), 
...
g2 <- ggplot(summary_data, aes(x = Block, y = Average_Performance_Before, group = Condition, color = Condition)) +
  geom_line() +
  geom_point(position = position_dodge(width = dodge_width)) +
  geom_errorbar(aes(ymin = Average_Performance_Before - se, ymax = Average_Performance_Before + se), 
                width = 0.4, position = position_dodge(width = dodge_width)) +
  theme_classic() +
  scale_color_manual(values = c("Decreasing" = colour.decreasing, "Increasing" = colour.increasing)) +
  labs(title = "Average Performance Before across Conditions with Standard Error",
       x = "Block",
  ...
  theme(plot.title = element_text(size=title_size))
print(g2)
ggsave(here('output','figures','05_Performance_Before_by_Block_by_Condition_with_SE.png'), 
       plot = g2, device = device, width = width, height = height, units = units, dpi = dpi)

# Visualize Interaction with Standard Error as Ribbon
dodge_width <- 0.2 # seperation between conditions
g3 <- ggplot(summary_data, aes(x = Block, y = Average_Performance_Before, group = Condition, color = Condition, fill = Condition)) +
  geom_line() +
  geom_point(position = position_dodge(width = dodge_width)) +
  ...
  theme(plot.title = element_text(size=title_size))
print(g3)
ggsave(here('output','figures','05_Performance_Before_by_Block_by_Condition_with_SE_as_Ribbon.png'), 
